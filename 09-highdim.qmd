...
- A **guided tour** instead finds target matrices that increase some numerical measure of "interestingness", known as a **projection pursuit index**.
...
As the tour rotates through different projections, the bulk of the data keeps roughly the same shape, but individual points move in and out of the data cloud.
...
sd_index <- function(mat) {
  z <- abs(mat[, 1] - median(mat[, 1])) / mad(mat[, 1])
  max(z)
}
```

Passing this index to `tourr::guided_tour()` produces a 1-dimensional tour that climbs towards the projection maximising it, shown in @fig-sd-guided-tour as an animated density plot, with the weight given to each variable in the current projection shown below.
...
#|   The top panel shows the density of the projected data; the bottom panel shows the weight given to each variable in the current projection.
...
if (!file.exists(gif_file)) {
  ...
  tourr::render_gif(
    as.matrix(Y),
    tourr::guided_tour(sd_index, d = 1),
    tourr::display_dist(col = "#0072B2"),
    gif_file = gif_file,
    frames = 150,
    width = 900,
    height = 700
...
The tour converges on a projection dominated almost entirely by `rainfall`, pulling a short right-hand tail away from the bulk of the distribution.
