...
This is exactly what a guided tour does,
...
### Finding outlyingness with a guided tour

Stahel--Donoho outlyingness is itself defined by a search over one-dimensional projections: for each candidate direction $\bm{a}$, compute the robust z-scores of the projected data, and keep track of the largest value seen so far.
This is exactly what a guided tour does, so we can use one to watch the search happen, rather than treating `adjOutlyingness()` as a black box.

We define a projection pursuit index that reproduces a simplified version of the outlyingness measure (without the skewness adjustment): given a 1-dimensional projection, it returns the largest robust z-score among the projected points.

```{r}
#| label: sd-guided-index
#| code-fold: false
#| dependson: aotizhongxin
...
Passing this index to `tourr::guided_tour()` produces a 1-dimensional tour that climbs towards the projection maximising it.
Rather than the usual density display, we use a custom `tourr` display that makes the search itself the point of the animation, tracing the index value reached at every frame.

```{r}
#| label: sd-guided-display
#| code-fold: true
#| dependson: sd-guided-index

# Step through a fixed sequence of bases (no plotting), recording the index
# value reached at every frame and the range of projected values seen --
# this lets the display below use stable axes from the very first frame.
trace_tour_path <- function(data, bases, index_f, apf = 1 / 10) {
  tour <- tourr::new_tour(data, tourr::planned_tour(bases), NULL)
  step <- tour(0)
  proj <- data %*% step$proj
  idx <- index_f(proj)
  xlim <- range(proj)
  repeat {
    ...
    proj <- data %*% step$proj
    idx <- c(idx, index_f(proj))
    xlim <- range(c(xlim, proj))
  }
  list(idx = idx, xlim = xlim)
}

# A tourr display with a rug of the projected points, a trace of the index
# value reached so far (log scale, since it blows up as the search isolates
# a single point), and the variable weights.
display_trace <- function(index_f, col = "#0072B2") {
  labels <- NULL
  idx_history <- numeric(0)
  frame_i <- 0
  total_frames <- NULL
  ylim_trace <- NULL
  xlim_rug <- NULL
  ...
  render_frame <- function() {
    par(mar = c(0, 0, 0, 0), cex.axis = 1.5, cex.lab = 1.6)
    graphics::layout(matrix(1:3, nrow = 3), heights = c(1, 3, 3))
  }
  ...
  render_data <- function(data, proj, geodesic) {
    ...
    plot(
      ...
      col = col,
      log = "y",
      xlim = c(1, total_frames),
      ylim = ylim_trace,
      xlab = "Frame",
      ylab = "Index"
    )
    points(frame_i, idx_val, pch = 19, col = col)
    ...
    text(1, -ax, labels, pos = 4, cex = 1.4)
  }
  list(
    init = init,
    ...
    render_target = function(...) NULL,
    set_total_frames = function(n) total_frames <<- n,
    set_ylim = function(r) ylim_trace <<- r,
    set_xlim = function(r) xlim_rug <<- r
  )
}
```

```{r}
#| label: fig-sd-guided-tour
#| code-fold: false
#| dependson: sd-guided-display
#| message: false
#| warning: false
#| out-width: 80%
#| fig-cap: >-
#|   A guided tour of the 11 standardised air quality and meteorological variables at Aotizhongxin, seeking the 1-dimensional projection that maximises the Stahel--Donoho outlyingness index.
#|   The top panel shows a tick for each observation's projected value; the middle panel traces the index value reached so far (log scale); the bottom panel shows the weight given to each variable in the current projection.

gif_file <- "gifs/highdim-sd-guided-tour.gif"
if (!file.exists(gif_file)) {
  set.seed(1)
  Ym <- as.matrix(Y)
  bases <- tourr::save_history(Ym, tourr::guided_tour(sd_index, d = 1))
  path <- trace_tour_path(Ym, bases, sd_index)

  disp <- display_trace(sd_index)
  disp$set_total_frames(length(path$idx))
  disp$set_ylim(range(path$idx) * c(0.9, 1.1))
  disp$set_xlim(path$xlim * 1.05)

  tourr::render_gif(
    Ym,
    tourr::planned_tour(bases),
    disp,
    gif_file = gif_file,
    frames = length(path$idx),
    width = 900,
    height = 900
  )
}
...
The index climbs steadily across the search, and the tour converges on a projection dominated almost entirely by `rainfall`, pulling a single observation far away from the bulk of the distribution.
This matches what `adjOutlyingness()` found earlier: the ten most outlying observations under this guided-tour index are the same ten hours identified above, including the run of hours early on 11 May.
Watching the tour arrive at this projection makes the earlier calculation concrete: Stahel--Donoho outlyingness is what a guided tour finds when the index it climbs is "how extreme is the most extreme point in this direction".
