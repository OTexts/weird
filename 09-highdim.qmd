...
FastPCS (@sec-fastpcs) found a run of extreme hours on 31 May whereas other methods only identified a few of these hours, while stray (@sec-stray) caught a transition late on 29--30 May that the covariance-based methods missed.
Stahel--Donoho outlyingness (@sec-stahel-donoho) found anomalies using one-dimensional projections, without our having to choose a projection, but it can miss anomalies that are only unusual in higher-dimensional projections.
...
`O3prep()` takes the data, a range of combination sizes (from `k1` to `K` variables per combination), and one or more outlier-detection methods, and works out which observations are flagged as outliers for every combination.
...
Because these methods are not directly comparable at a single shared tolerance level, `O3prep()` takes a separate tolerance argument for each one, and the defaults tend to flag far more outliers than are useful in practice, so these are usually worth adjusting for the dataset at hand.
The result is passed to `O3plotT()`, which draws an O3 plot comparing up to three tolerance levels for a single method, or `O3plotM()`, which compares up to six different methods against each other.
...
Instead, we restrict attention to five variables that recurred throughout our earlier discussion --- `co`, `no2`, `pm2_5`, `so2` and `dew_point` --- and compare all five methods available in `OutliersO3` across every combination of one to five of them.
...
o3 <- OutliersO3::O3prep(
  as.data.frame(Y[, c("co", "no2", "pm2_5", "so2", "dew_point")]),
...
#| fig-asp: 0.5
#| fig-cap: >-
#|   O3 plot comparing five outlier-detection methods --- FastPCS, mvBACON, Stahel--Donoho outlyingness (adjOut), DDC and MCD --- across all combinations of one to five of five key air quality variables (`co`, `no2`, `pm2_5`, `so2`, `dew_point`) at Aotizhongxin.
...
Most flagged cells are light or mid blue, meaning only one or two methods agree for that combination, and full agreement is rare: there is a single red cell in the whole plot, for the combination of `no2` and `so2`, marking the one observation that all five methods flag as an anomaly at once.

We can make this more precise by counting, across every combination and every method, how many times each observation was flagged as an outlier.
...
az_aq_proj |>
  ...
  mutate(n_flagged = out_table$n_flagged) |>
  select(year:hour, n_flagged) |>
...
The single red cell in @fig-o3plot belongs to 11am on 31 May, which is flagged in `r out_table$n_flagged[1]` of the `r o3$nw * 5` combination/method pairs checked --- more than half of them, and enough that all five methods agree on it for at least one combination of variables.
Even mvBACON, the most conservative of the five methods here, agrees on both episodes: its `r o3_plots$nOut[["BAC"]]` flagged hours include several from 22 and 31 May, even though it only reaches full five-method agreement for that single hour.
This is the value of comparing methods this way: the identity of the most extreme hours is robust to the choice of method and tolerance, even though individual methods still disagree on the details.
...
This is the central lesson of high-dimensional anomaly detection: the curse of dimensionality means that anomalies can hide in some views of the data and stand out in others, so no one projection or method should be trusted on its own.
