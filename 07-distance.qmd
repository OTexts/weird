...
This method is implemented in the `cellWise::DDC()` function. Here we apply it to the French male mortality data, with the data matrix containing the mortality rates for each year (rows) and age (columns). The standardised residuals from the DDC algorithm are plotted as a heatmap in @fig-ddc-mx, with red indicating large positive residuals, blue indicating large negative residuals, and grey indicating small residuals.
...
```{r}
#| label: fig-ddc-mx
#| fig-cap: "DDC algorithm applied to the French male mortality data. The standardised residuals from the DDC algorithm are plotted as a heatmap, with red indicating large positive residuals, blue indicating large negative residuals."
#| message: false
ggplot(mx_ddc) +
