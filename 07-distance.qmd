...
A popular way of using $k$-nearest neighbours for anomaly detection is via local outlier factors [@lof]. This is similar to the idea discussed in @sec-kdesurprisals of finding points with low probability density estimates, in that it is designed to find points in areas with few surrounding observations.
...
Let's apply the idea to the `duration` variable in the Old Faithful data set. We will use the `lof_scores()` function to compute the local outlier factors for each eruption, with $k=120$ nearest neighbours. The value of $k=120$ has been chosen after experimenting with various values.
