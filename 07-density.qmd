...
The OLS estimation of this model was described in @sec-bacon-reg. Recall that the fitted values are given by
$$
  \hat{\bm{y}} = \bm{H}\bm{y},
$$
...
We return to the Shiraz wine reviews introduced in @sec-bacon-reg, fitting the same linear regression model with `log(price)` as the response and `points` as the predictor.
