...
$$
  y \mid \bm{x} \sim N(\bm{x}_+^\intercal\bm{\beta}, \sigma^2),
$$ {#eq-gaussian-regression}
where $\bm{x}_+ = [1, \bm{x}]^\intercal$ is a $(p+1)$-dimensional vector containing a 1 in the first position and the predictors in the remaining positions, and $\bm{\beta}$ is a $(p+1)$-dimensional vector of regression coefficients.
...
Note that the residuals have the distribution $\bm{e}\mid\bm{X} \sim N(\bm{0}, \sigma^2(\bm{I} - \bm{H}))$, which is not quite the same as the distribution of the errors given by @eq-error-distribution. However, as $n\rightarrow\infty$, $\bm{H}\rightarrow\bm{I}$, so the two distributions are asymptotically equivalent. Often, we need standardised residuals, which are obtained by dividing each residual by its estimated standard deviation, giving
...
$$
so we can standardise each LOO residual to obtain
$$
...
The LOO surprisals obtained from this model are shown in @fig-shiraz-regression-scores, using the same colours as @fig-shiraz-regression to indicate whether the observation is below, within, or above, the 95% prediction interval.

```{r}
#| label: fig-shiraz-regression-scores
#| fig-cap: Residuals and surprisals for the Shiraz data using a linear regression model. Points are coloured to match the 95% prediction intervals in @fig-shiraz-regression.
#| fig-height: 6
#| fig-asp: 0.8
wine_aug <- wine_aug |>
  mutate(
