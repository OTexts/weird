...
$$
p_i = \int f(\bm{u}) 𝟙(-\log f(\bm{u}) \ge s_i) d\bm{u} = \int f(\bm{u}) 𝟙(f(\bm{u}) \leq f(\bm{y}_i)) d\bm{u},
$$ {#eq-pi-density}
...
This approach allows for anomalies to occur anywhere in the distribution, not just in the tails. For example, anomalies may lie between modes in a multimodal distribution, or in otherwise low-density regions of the distribution. The surprisal approach coverts the problem of working with a potentially complicated distribution into a problem of working with a simple univariate distribution on the surprisal scale, where large values correspond to anomalies.
...
where $\bm{\beta}$ is a $p$-dimensional coefficient vector. In matrix form, where $\bm{X}$ is an $n\times p$ matrix with rows equal to the predictor variables for each observation, this is
$$
...
Thus the standardised LOO residuals (also known as "externally studentized residuals") are given by
$$
...
#| warning: false
#| fig-cap: Log price of Shiraz as a function of points, with 95% prediction intervals. The points are horizontally jitted to reduce overplotting. Points outside the prediction intervals are coloured.
...
$$
where $\mu = \text{E}(y | \bm{x})$ denotes the conditional mean, $\ell()$ is a link function, and each $g_k$ function is smooth. If $f$ is Normal, $\ell$ is the identity, and $g_k(u) = \beta_k u$, then this reduces to the linear Gaussian model (@eq-gaussian-regression).
