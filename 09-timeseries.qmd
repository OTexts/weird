...
The `hampel_anomalies()` function implements this algorithm. The code below applies the algorithm to all 172 time series in `fr_mortality`. The `bandwidth` argument corresponds to $h$, while the `k` argument corresponds to $k$ above.
...
We will apply this idea to find unusual observations in US weekly mortality data between 2015 and 2024, obtained from the @HMD. @fig-stmf shows the data on five age groups. There appears to be no seasonality in the 0--14 age group, but all other age groups have seasonal peaks around the start of January each year due to winter illnesses. There is a noticeable change in mortality rates during 2020, 2021 and the start of 2022. This is due to COVID-19.
...
The `.fitted` column contains $\hat{m}_t$, while `.resid` column contains estimates of $y_t - \hat{m}_t$, where $\hat{m}_t$ takes account of any seasonality in the data. We will estimate $s_t$ using a robust estimate of standard deviation based on the IQR, with an adjustment so that it is equivalent to a standard deviation if the data were normally distributed. (Note that we are using $s$ to denote the standard deviation of a series, not the surprisals, here.)
...
At this point, we could identify a point as an anomaly if $|e_t|$ is greater than $k$, as was done with the Hampel identifier. But we will use surprisals with probabilities calculated using a GPD, as it is a safer calculation making fewer assumptions about the distribution of the residuals.
...
stmf_scores |> filter(prob < 0.02) |> arrange(prob)
```
...
#| warning: false
#| message: false
#| fig-cap: US weekly mortality rates identified as anomalies using surprisal scores obtained from applying STL to each series.
#| dependson: stmf_scores2
us_stmf |>
  autoplot(Mortality) +
  facet_grid(`Age group` ~ ., scales = "free_y") +
  geom_point(data = stmf_scores |> filter(prob < 0.02))
```

A total of 55 observations have surprisal probabilities less than 0.02, with the most extreme being in weeks 15--16 of 2020, in the elderly age groups. Although the test can pick up unusually low or unusually high mortality rates, only one of the 55 identified anomalies is for low mortality, and that is for 85+ year olds in Week 3 of 2020, just before COVID-19 cases arrived in the United States (but perhaps after people starting taking some precautions). There are four anomalous peaks in the plot, centred on the following dates:
...
Some of these periods affected some age groups more than others. This analysis treats each series separately, and seeks to find anomalies within each series, regardless of the behaviour of related series.

### Multivariate time series

To take account of all series simultaneously, we need to take a multivariate approach, where we are looking for time periods that are anomalous across multiple series. One way to do this is to first take principal components of the series, and then apply the preceding procedure to the series of first principal component scores.

We will apply this to the US weekly mortality data. First we convert the series to logs as they are on vastly different scales, then we transform the data to wide form. Finally, we compute the first principal component of the resulting matrix.
...
Now we can apply an STL model to find anomalies in this series (plotted in @fig-us_stmf_pcs). There is no need to scale the residuals because there is only one series.
...
pc_anomalies <- pcs |>
  ...
  select(-.model, -.innov) |>
  mutate(
    prob = surprisals(.resid, distribution = dist_normal(), approximation = "gpd")
  ) |>
  filter(prob < 0.05)
pc_anomalies
```
