...
Let $y_{(k)}$ denote the $k$th smallest observation, $k=1,\dots,n$. These ordered values are known as "**order statistics**". Then the sample quantile $\hat{Q}(p)$ is given by linear interpolation between consecutive order statistics. That is, if $k^+= p(n+1-2a) +a$ for some constant $a$, and $k = \lfloor k^+ \rfloor$ is the integer part of $k^+$, then
$$
...
$$ {#eq-POT3}
When the distribution $F$ satisfies the FTG theorem, then [@coles2001introduction, p75] $H$ is a **Generalised Pareto Distribution** (GPD) defined by
  $$
