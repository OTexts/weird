...
$$
  h = 1.059 s n^{-1/5}
$$ {#eq-nrr}
but because we want our results to be robust to outliers, we will replace $s$ by the robust $Q_n$ estimate, resulting in
$$
  h = 1.059 Q_n n^{-1/5}.
$$ {#eq-rnrr}
...
$$ {#eq-opth}
Replacing $\sigma$ by the sample standard deviation gives us the normal reference rule @eq-nrr; while replacing $\sigma$ by the robust estimate $Q_n$ gives use the robust normal reference rule @eq-rnrr.
...
Another popular bandwidth choice is the "plug-in" bandwidth [@SJ91], obtained by replacing $R(f'')$ in @eq-opth by an estimate based on the data. This is computed by `bw.SJ()` and can be used with `geom_density()` and `density()` by setting `bw = "SJ"`. For most data sets, this will give a smaller value than @eq-nrr.
