...
The central limit theorem (CLT) is one of the most important results in statistics. There are several variations of the theorem; the simplest version states that the sum of many iid random variables, with finite mean and variance, is approximately Normally distributed.
...
Raw spacings are difficult to compare across the distribution: where the density is high they tend to be small, and where the density is low they tend to be large. To put them on a common scale, we can apply the **probability integral transform**. If the observations are iid from a continuous distribution $F$, then $u_i = F(y_{(i)})$ are distributed as uniform order statistics on $[0,1]$, and each transformed spacing $F(y_{(i+1)}) - F(y_{(i)})$ has expected value $1/(n+1)$ regardless of position in the distribution. A transformed spacing substantially larger than $1/(n+1)$ therefore signals an unusually large gap at that location, which may indicate an anomalous observation. Methods based on this idea are discussed in @sec-gap-tests.
