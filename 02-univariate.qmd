...
#| label: fig-chisq
#| message: false
#| fig-cap: Probability density functions for three $\chi^2$ distributions. As the degrees of freedom $k$ increases, the distribution becomes more symmetric, and approximates the Normal distribution N$(k,2k)$.
dist_chisq(c(1, 2, 5)) |>
  gg_density() +
  labs(x = "y", y = "Probability Density Function: f(y)") +
...
The **t** distribution was first described by the English statistician and chemist, William Gosset, who worked for the Guinness brewery in Dublin, Ireland, and published under the pseudonym "Student". It arises naturally as the ratio $Z/\sqrt{S/k}$, where $Z\sim \text{N}(0,1)$ and $S\sim \chi^2_k$ are independent random variables, and is commonly used in variations of $t$-tests. It takes one parameter, the degrees of freedom $k$. The pdf is given by
$$
...
The $\Gamma$ (or **Gamma**) distribution is a generalisation of the $\chi^2$ distribution, and is used in many statistical models. There are several parameterisations. Here, we use the shape/rate parameterisation which uses the shape $\alpha$ and the rate $\beta$. The pdf is given by
$$
...
```{r}
#| label: fig-gamma
#| fig-cap: Probability density functions for three $\Gamma$ distributions. As $\alpha$ increases, the distribution converges to a N$(\alpha/\beta, \alpha/\beta^2)$ distribution.
...
$$
where $f(y;\mu_j,\sigma_j^2)$ is the pdf of the Normal distribution with mean $\mu_j$ and variance $\sigma_j^2$. The mean and variance of the mixture distribution are given by
$$
...
The **probability integral transform** is a useful result that allows us to transform any continuous random variable into a uniform random variable. If $Y$ is a continuous random variable with cdf $F$, then the random variable $U = F(Y)$ has a U(0,1) distribution. This is because $\Pr(U \le u) = \Pr(F(Y) \le u) = \Pr(Y \le F^{-1}(u)) = F(F^{-1}(u)) = u$ for $0 < u < 1$. The inverse is also true: if $U$ is a U(0,1) random variable, then the random variable $Y = F^{-1}(U)$ has distribution $F$. A useful consequence of this result is that it is possible to simulate from any continuous distribution by first simulating from a U(0,1) distribution, and then applying the inverse cdf of the desired distribution.
...
The central limit theorem (CLT) is one of the most important results in statistics. There are several variations of the theorem; the simplest version states that the sum of many independent and identically distributed (iid) random variables, with finite mean and variance, is approximately Normally distributed.
...
It is because of the CLT that a $\chi^2_k$ distribution is approximately Normal for large $k$. (Recall that the sum of the squares of $k$ independent N(0,1) random variables has a $\chi^2_k$ distribution.)
...
The **order statistics** of a sample $y_1,\dots,y_n$ are the values rearranged in ascending order, denoted $y_{(1)} \le y_{(2)} \le \cdots \le y_{(n)}$. The $i$th **spacing** is the gap between consecutive order statistics,
$$
  d_i = y_{(i+1)} - y_{(i)}, \quad i = 1, \ldots, n-1.
$$ {#eq-spacing}

If the sample is iid from a uniform distribution, then for large $n$ the spacings are approximately iid exponential random variables with mean $1/(n+1)$, i.e., $\text{Exp}(n+1)$. A spacing substantially larger than $1/(n+1)$ therefore signals an unusually large gap at that location, which may indicate an anomalous observation.

For other non-uniform distributions, raw spacings are difficult to compare across the distribution: where the density is high they tend to be small, and where the density is low they tend to be large. To put them on a common scale, we can apply the **probability integral transform** (@sec-pit). If the observations are iid from a continuous distribution $F$, then $u_i = F(y_{(i)})$ are distributed as uniform order statistics on $[0,1]$, and the above results apply to the transformed spacings $F(y_{(i+1)}) - F(y_{(i)})$.
...
A quantile of a continuous univariate distribution is a point that divides the sample space into two regions, based on the probability of observations falling below or above the quantile value. If $Q(p)$ denotes the quantile corresponding to probability $p$, then $\Pr(Y \le Q(p)) = p$ and $\Pr(Y > Q(p)) = 1-p$.

```{r}
#| label: fig-quantile
#| fig-cap: The 0.8 quantile of a $\chi^2_5$ distribution is the value $Q(0.8) = 7.29$ such that $\Pr(Y \le 7.29) = 0.8$. The shaded area is 0.8, and the probability of observing a value greater than 7.29 is 0.2.
dist <- dist_chisq(5)
...
The "interquartile range" (IQR) is the difference between the upper and lower quartiles. It is a measure of the spread of the distribution, and is often used as a robust alternative to the standard deviation. For a Normal distribution, the IQR equals `r sprintf("%.3f", normal_iqr)` times the standard deviation (and hence the standard deviation is approximately `r sprintf("%.3f", 1/normal_iqr)` times the IQR).
...
The sample quantile $\hat{Q}(p)$ is given by linear interpolation between consecutive order statistics. That is, if $k^+= p(n+1-2a) +a$ for some constant $a$, and $k = \lfloor k^+ \rfloor$ is the integer part of $k^+$, then
$$
  \hat{Q}(p) = (1-\gamma) y_{(k)} + \gamma y_{(k+1)},
$$
where $\gamma = k^+ - k$ is the remainder. Most of the different methods for estimating quantiles differ only in the value of $a$.
...
where $f(Q(p)) > 0$ and $f(y)$ is the density of the underlying distribution [@van2000asymptotic, p. 308]. This is an asymptotic result, which assumes $np$ is large, and holds for all types of sample quantiles.
...
Now we will apply @eq-kde to the first ten Old Faithful eruption durations from 2021 that are in the `oldfaithful` data set. The kernel density estimate can be visualised as a sum of kernel functions centred over each observation with width determined by $h$, and height given by $K_h(0)/n$.
...
When $h$ is too small, the density estimate is too rough, with many peaks and troughs. When $h$ is too large, the density estimate is too smooth and we fail to see any features in the data. A popular choice for $h$ is the "normal reference rule" [@Silverman1986, p. 45] given by
$$
  h = 1.059 s n^{-1/5}.
$$ {#eq-nrr}
...
$$
  \int \text{E}\left[(\hat{f}(y) - f(y))^2\right] dy \approx
...
The optimal overall bandwidth is obtained by minimising the MISE. This can be calculated by differentiating @eq-mise with respect to $h$ and setting the derivative to zero, yielding
$$
...
$$ {#eq-nrd}
Silverman's "rule-of-thumb" is similar, but it replaces 1.06 in @eq-nrd by 0.9, because Silverman argued that most densities are not as smooth as a Normal density, and so a smaller bandwidth is required. This is the default bandwidth used by `geom_density()` and `density()`.
...
A final option in `kde_bandwidth()` is `method = "lookout"`. We will defer a description of that method until @sec-mvkde.
...
$$
  \Pr\left\{ \frac{(M_n - a_n)}{b_n} \le z \right\} \rightarrow G(z) \quad \text{as} \quad n \to \infty,
$$
...
The three types of limits correspond to different forms of the tail behaviour of $F$.
  ...
  * When $F$ has heavy tails (such as polynomial decay), then $G$ is a Fréchet distribution. One example is when $F$ itself is a Fréchet distribution, for example.

We refer to $F$ as being in the "domain of attraction" of the corresponding limit distribution. For example, the Normal distribution is in the domain of attraction of the Gumbel distribution.
...
According to the FTG theorem, the distribution of the maximum can be approximated by a Gumbel distribution. We can find the parameters of the Gumbel distribution by estimating them from simulated maximums. We simulate 2000 maximums (each from 1000 N(0,1) draws), and fit a Gumbel distribution to the resulting data. The resulting Gumbel distribution is shown in @fig-evdexample (in orange), along with the true distribution of the maximum given by @eq-maxn01 (in blue). The approximation is so good, that it is hard to distinguish the two distributions.

```{r}
#| label: "fig-evdexample"
#| fig-cap: "Distribution of the maximum of 1000 N(0,1) draws. Here we have simulated 2000 such maximums and shown the estimated GEV distribution (in orange), along with the true distribution (in blue). The approximation is so good, that it is hard to distinguish the two distributions."
n <- 1000
...
When the distribution $F$ satisfies the FTG theorem, then [@coles2001introduction, p. 75] $H$ is well-approximated by a **Generalised Pareto Distribution** (GPD) defined by
  ...
  $$ {#eq-POT}
where the domain of $H$ is $\{y > 0 : 1 + \xi y/\sigma_u > 0 \}$, and $\sigma_u = \sigma_G + \xi(u- \mu_G)$. The GPD parameters are determined from the associated GEV parameters. In particular, the shape parameter $\xi$ is the same in both distributions.
...
Specifically, let $D_k = Y_{(k+1)} - Y_{(k)}$ denote the spacing between successive order statistics. If $F$ is in the domain of attraction of the Gumbel distribution, then, for fixed $r$, the upper spacings $D_{n-1}, D_{n-2}, \dots, D_{n-r}$ are asymptotically independent and approximately exponentially distributed, with $D_{k}$ having mean $b_n/(n-k)$. (Here $b_n$ is the same scaling constant that was used in the FTG theorem.)
