...
Most readers will have first come across anomaly detection using boxplots, invented by John Tukey as a quick summary of medium-sized data sets [@Tukey75;@Tukey1977-xd;@wickhamboxplots]. Tukey never proposed that boxplots be a generic anomaly detection method, but they have become widely used for this purpose, and are often the first method that people think of when they want to identify anomalies in univariate data.
...
In Tukey's original version, the ends of the box were determined by the letter values corresponding to the fourths, and the middle line within the box showed the median. This is what is produced by the base R function `boxplot()`. In the `ggplot2` version of the boxplot shown here, the ends of the box are the quartiles computed using `type = 7`.
...
Whichever variation is used, roughly half of all observations lie within the box. The length of the box is an estimate of the "interquartile range" (IQR). Any points more than 1.5 IQR outside the box are shown as separate points in the plot. The "whiskers" that extend out each side of the box show the range of the remaining points.

Originally, Tukey proposed two levels of outliers --- those more than 1.5 IQR beyond the box were labelled "outside" values, while those more than 3 IQR beyond the box were labelled "far out" values. Most software implementations of boxplots do not distinguish between these groups. When a boxplot is used for anomaly detection, all points outside the whiskers are called "anomalies" or "outliers", even though they may not be genuine anomalies.
...
This is an improvement on the original boxplot approach, and it finds the true anomalies in the `cricket_batting` data and the `n01b` data. But it also identifies many false anomalies in the other data sets, especially those that are not Normally distributed. The assumption of Normality means that the method is not a good choice for most data sets.
...
Letter value plots were not designed to detect anomalies, but to be a useful data visualisation tool for univariate distributions with large numbers of observations. So the display of legitimate observations in the tails of the distribution is by design, not a flaw.
...
Of course, by definition, 1% of points will lie outside the 99% region, so the points shown are not necessarily anomalies, just observations that occur in the lower probability regions of the space. By setting `show_anomalies = TRUE`, some of the points are highlighted as potential anomalies, using the method described in @sec-kdesurprisals.

The idea naturally extends to higher dimensions. @fig-hdr-faithful2 shows the bivariate HDR boxplot of the duration and waiting times for the Old Faithful data.
...
#| fig-cap: "Bivariate HDR boxplot of the duration and waiting times of Old Faithful eruptions between 2017 and 2023. Individual points are coloured according to the HDR region in which they fall."
oldfaithful |>
