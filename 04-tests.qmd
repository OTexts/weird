...
We will only consider univariate methods in this chapter. There have been multivariate parametric tests for anomaly detection, but they are much less widely used and are also best avoided.
...
2. Old Faithful eruption durations between 2017 and 2023, also discussed in @sec-scatterplots. For these data, there were three extreme anomalies --- with durations up to 1 minute --- and several "inliers" in an area of low density between 140 and 180 seconds.
...
All methods considered in this chapter assume the underlying data follow a Normal distribution. In our examples, only #4 is from a Normal distribution, so the methods should work well in that case, but perhaps not in the other cases.
...
In fact, we can be more accurate than this. If our $n$ observations come from a Normal distribution, then the z-scores follow (approximately) a $\text{t}_{n-1}$ distribution. (The approximation arises because we replace the mean and standard deviation by their sample estimates.) When we identify anomalies as those points with $|z|>c$, then the probability of finding at least one spurious anomaly is the probability of the maximum being above $c$ or the minimum being below $-c$. Since the distribution is symmetric, these probabilities are the same. Let $M_n$ be the maximum of $n$ z-scores computed from the data; then $\Pr(M_n \le c) = (F_{t}(c; n-1))^n$ where $F_{t}(y; n-1)$ is the cumulative distribution function of a $t$ distribution with $n-1$ degrees of freedom. So the probability of at least one spurious anomaly is
$$
  1 - (F_{t}(c; n-1))^{2n}.
$$ {#eq-spuriousz}
...
The `chauvenet_anomalies()` function can be used to implement this test. For our six examples, it gives similar results to those above for `peirce_anomalies()` (with two additional spurious anomalies for the $\text{t}_3$ example, and one additional spurious anomaly for the $\chi^2_4$ example.)
...
The z-score tests above all measure the distance of an observation from the centre of the distribution. A different approach looks instead at the **gaps** between consecutive order statistics (see @sec-gaps): if a value is anomalous, we expect a noticeably large gap between it and its nearest neighbour in the sorted data. @Dixon1950 introduced the first widely-used test of this type.
...
Dixon's test only examines the gap adjacent to the minimum or maximum observation. @Tukey1977-xd proposed looking for large gaps anywhere in the sorted data, while @Wainer1978 formalised this into a practical method for determining whether any such spacing is unexpectedly large for its position in the distribution. The method is best thought of as detecting unusually large **separations** or **holes** in a one-dimensional data set. A large tail gap may suggest an outlier; a large interior gap usually suggests a separation between two groups rather than a single anomalous observation.
...
Raw spacings are not directly comparable across the sorted sample. Even for data from a single unimodal distribution, spacings tend to be small near the centre of the distribution, where observations are dense, and larger in the tails, where observations are sparse.

Tukey’s *gapping* scheme addresses this by weighting each gap according to its position in the ordered data. Specifically, the gap between $y{(i)}$ and $y{(i+1)}$ is assigned weight
$$
  a_i = i(n-i), \qquad i = 1, \ldots, n-1,
$$ {#eq-ws-weight}
which is largest near the middle of the sample and smallest at the extremes. This emphasises unusually large gaps in the centre, where observations are normally close together, and downweights gaps in the tails, where larger spacings are expected.

@Wainer1978 developed this idea into a practical diagnostic by transforming and standardising the weighted gaps. They proposed using the square root of the weighted gaps,
$$
  h_i = \sqrt{a_i d_i},
$$
...
$$ {#eq-ws-score}
Large values of $z^*_i$ identify gaps that are large relative to other gaps after allowing for their position in the sorted data. Tukey suggested $z^*_i > 2.25$ as an exploratory cut-off; @Wainer1978 showed by simulation that, under a Gaussian distribution, this corresponds to a per-gap false positive rate of about 3%. Because there are $n-1$ gaps, this is not a 5% test for the whole data set. If many gaps are inspected, the chance of at least one crossing the threshold can be much larger than the per-gap rate.

The code below constructs a data frame of **gaps** between batter averages.

```{r}
#| label: fig-ws-gaps
#| fig-cap: "Tukey--Wainer--Schacht gap scores for cricket batting averages (batters with more than 25 innings). Each point is plotted at the upper end of a gap, so the score shown at a batting average is for the gap between that batter and the next lower batting average. The dashed line marks Tukey's exploratory threshold of 2.25."
...
The largest gap of `r sprintf("%.1f", batting_gaps |> filter(ws > 2.25) |> slice_max(spacing, n = 1) |> pull(spacing))` should be interpreted as the separation between Adam Voges (who has the second highest batting average) and Don Bradman, not as a score belonging to Bradman alone. This illustrates that the method detects separations, not individual anomalous observations.

### Significance testing {-}

The Tukey--Wainer--Schacht score does not yield a direct $p$-value. A related way to construct a formal gap test is to calculate the uniform spacings (see @sec-gaps) of the data after transforming them using the fitted distribution function $F$:
$$
...
In practice, $F$ is unknown. The example below uses a robustly fitted Normal distribution, with the median and MAD as estimates of location and scale. This makes the fit less sensitive to the possible outlier, but the resulting test is still only an approximate parametric test.
...
batting_gaps |>
  ...
  filter(p_value < 0.05) |>
  select(lower_player, lower_average, upper_player, upper_average, spacing, p_value)
```
...
Finally, we apply the same approach to the other five examples considered in this chapter. Here we associate the p-value with the larger observation in each gap pair.
...
Like all methods in this chapter, both the normalised gap score and the transformed-gap test depend on an assumed null distribution, and so the results depend on the adequacy of the model. Related extreme-spacing ideas underlie the HDoutliers and stray algorithms discussed in @sec-distance-methods, where gaps in nearest-neighbour distances are used rather than gaps in the raw univariate data.
...
Only Z scores found the three (and only the three) clear anomalies in the Old Faithful data, and most failed to find either of the artificial anomalies in `n01b`. All tests found at least one spurious anomaly in the large simulated data sets with no real anomalies.

As noted at the start of this chapter, anomaly detection methods based on assumed data distributions are unreliable. Until about 1975, they were the only viable methods given the lack of computing facilities available, because they could be implemented using tables and hand calculations. However, there is little justification for continuing to use such methods.
