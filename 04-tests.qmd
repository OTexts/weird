...
The Tukey--Wainer--Schacht score does not yield a direct $p$-value. A related way to construct a formal gap test is to transform the data to a scale on which the null distribution is uniform. If the data are iid from a continuous distribution $F$, and if $F$ is known, then
$$
  u_i = F(y_{(i)})
$$
are order statistics from a uniform distribution on $[0,1]$. So unusually large gaps in the original data become unusually large uniform spacings
$$
  \delta_i = F(y_{(i+1)}) - F(y_{(i)}), \quad i = 1, \dots, n-1.
$$
Each of these internal spacings has expected value $1/(n+1)$. If
$$
  \Delta_{\max} = \max_i \delta_i,
$$
then an extreme-value approximation for the largest of the $n-1$ spacings gives
$$
  t = (n+1)\Delta_{\max} - \log(n-1)
$$
as approximately standard Gumbel under the assumed distribution $F$. This gives the approximate upper-tail probability
$$
  p \approx 1 - \exp(-e^{-t}).
$$ {#eq-gap-pvalue}
