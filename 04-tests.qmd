...
$$
be the sorted observations, and define the adjacent spacings
$$
  d_i = y_{(i+1)} - y_{(i)}, \qquad i = 1, \ldots, n-1.
$$

Raw spacings are not directly comparable across the sorted sample. Even for data from a single unimodal distribution, spacings tend to be small near the centre of the distribution, where observations are dense, and larger in the tails, where observations are sparse. Tukey's correction gives more weight to gaps in the centre and less weight to gaps in the tails:
$$
  a_i = i(n-i), \qquad i = 1, \ldots, n-1.
$$ {#eq-ws-weight}
The factor $a_i$ is the product of the number of observations below the gap and the number above it. It is largest near the middle of the ordered sample and smallest at the two extremes. The weighted gap is then
$$
  h_i = \sqrt{a_i d_i}.
$$
The weighted raw gaps are strongly right-skewed, and taking square roots makes their distribution more nearly symmetric. Finally, compare each $h_i$ with a robust estimate of the typical weighted gap size, the 25% trimmed mean of the $h_i$ values:
$$
  z^*_i = \frac{h_i}{\bar h_{T25}}.
$$ {#eq-ws-score}
Large values of $z^*_i$ identify gaps that are large relative to other gaps after allowing for their position in the sorted data. Tukey suggested $z^*_i > 2.25$ as an exploratory cut-off; @Wainer1978 found by simulation that, under a Gaussian distribution, this corresponds to a per-gap false positive rate of about 3%. Because there are $n-1$ gaps, this is not a 5% test for the whole data set. If many gaps are inspected, the chance of at least one crossing the threshold can be much larger than the per-gap rate.
...
Restricting attention to scores above the exploratory threshold of 2.25, the five largest gaps are shown below.
