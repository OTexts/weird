...
df <- tibble(
  ...
  s = -log(fy),
  p = surprisals_prob(y, distribution = den, approximation = "none")
)
palpha <- 0.1
p1 <- df |>
  ggplot(aes(x = y, y = fy)) +
...
tibble(
  ...
  p = surprisals_prob(y, distribution = dist_normal(), approximation = "none")
)
```
...
The `surprisals_prob()` function has an approximation argument set to `"gpd"` by default, specifying if we want to use the GPD approximation in computing surprisal probabilities. The argument `threshold_probability` is used to specify $\beta$, with the default value of 0.1.
...
norm_prob <- tibble(
  ...
  pt =     surprisals_prob(y, distribution = dist_student_t(4), approximation = "none"),
...
of_surprisals <- oldfaithful |>
  mutate(
    ...
    prob = surprisals_prob(duration, loo = TRUE),
  )
...
of |>
  mutate(
    ...
    prob = surprisals_prob(of, loo = TRUE)
  ) |>
  filter(prob < 0.005) |>
...
cricket_batting |>
  filter(Innings > 20) |>
  mutate(prob = surprisals_prob(Average, loo = TRUE)) |>
  filter(prob < 0.001) |>
...
n01b |>
  mutate(prob = surprisals_prob(y, loo = TRUE)) |>
  filter(prob < 0.05) |>
  arrange(prob)
```
...
n01 |>
  select(v1) |>
  mutate(prob = surprisals_prob(v1, loo = TRUE)) |>
  filter(prob < 0.001) |>
  arrange(prob, v1)
set.seed(1)
