...
For any symmetric univariate distribution, $p_i = 2[1 - F(c+|y_i-c|)]$, where $F$ is the corresponding distribution function centred at $c$. Therefore, under the Normal distribution $N(\mu, \sigma^2)$, $p_i = 2[1-\Phi(|y_i-\mu|/\sigma)]$, where $\Phi$ is the standard Normal distribution function.
...
A linear model can be estimated in R using the `stats::lm()` function. The `broom::augment()` function will compute the residuals (named `.resid`), the standardised residuals (named `.std.resid`), and the leverage values (names `.hat`).
...
#| code-fold: false
wine_aug |>
  mutate(prob = 1 - rank(loo_surprisals) / NROW(wine_aug)) |>
...
Now there are `r english::words(no)` points with probabilities less than 0.005, two at 155, and three above 300.
...
Now, `r english::words(sum(surprisals_prob(of, loo = TRUE) < 0.001))` anomalies are identified, with both of them having infinite LOO surprisals. We can visualise them in an HDR scatterplot, shown in @fig-ofpot2.
