...
The `surprisals()` and `surprisals_prob()` functions each have a `loo` argument for specifying whether we want LOO surprisals used in the computation.
...
#| code-fold: false
shiraz <- shiraz |>
  mutate(prob = surprisals_prob(fit_wine, approximation = "empirical"))
shiraz |>
...
Now, `r english::words(sum(surprisals_prob(of) < 0.001))` anomalies are identified, with both of them having infinite LOO surprisals. We can visualize them in an HDR scatterplot, shown in @fig-ofpot2.
