...
Some anomaly detection methods use tools based on clustering, and so we will need to understand some basic concepts of clustering, including how to represent clusters using graphs and dendograms.
...
When a cluster merges with another cluster, we say that one cluster "dies", while the other continues to exist, but now includes the points from both original clusters. The value of $\delta$ at which this happens is called the "death diameter" of the cluster that dies. With $n$ observations, there are $n$ initial clusters, but only $n - 1$ merges are needed to end up with one final cluster. So we get $n - 1$ finite death diameters.

For our simple example, the death diameters are shown in @fig-deathdiameters.

```{r}
#| label: fig-deathdiameters
#| echo: false
#| fig-cap: "Death diameters of the point cloud in @fig-graph. Each line denotes a cluster, with length equal to its death diameter."
#| fig-width: 8
#| fig-height: 1.6
#| fig-asp: 0.2
#| out-width: "100%"
...
plot_barcode(ph1) + xlab("Death Diameter") + guides(col = "none")
```

@fig-deathdiameters shows a bar plot of the cluster death diameters for the point cloud shown in @fig-graph. The shortest diameter corresponds to when $A$ and $F$ joined to form an edge --- when one of the separate vertex clusters died. The largest diameter corresponds to when all points became connected, when the $BH$ edge cluster died and joined the rest of the data.

Clusters that continue for a large range of $\delta$ represent interesting structural properties of the data that are of interest to us. These are denoted by the longest lines in @fig-deathdiameters.

### Hierarchical clustering

The above description of clusters and their merges as $\delta$ increases is exactly the same as the process of **hierarchical clustering**. In hierarchical clustering, we start with each observation as its own cluster, and then repeatedly merge the two closest clusters together until we have one final cluster. The distance between two clusters is defined as the smallest distance between any point in one cluster and any point in the other cluster. This is known as **single-linkage hierarchical clustering**.

Hierarchical clustering is usually displayed as a **dendrogram**. In a dendrogram:

- Each observation starts as a leaf.
- Branches join when two clusters merge.
- The vertical axis shows the distance (diameter) at which the merge occurs.

So the heights of the joins in a single-linkage dendrogram are precisely the death diameters.

If `X` is a matrix or data frame of scaled observations, you can compute the single-linkage dendrogram and the merge distances like this:

```{r}
d <- dist(X)
hclust(d, method = "single") |> plot(ylab = "Death diameter", xlab = "Observation")
```

The vector `hc$height` contains the merge distances (one for each of the $n - 1$ merges). These are the death diameters. If you want the death diameters as an unordered set (to take quantiles, for example), you can work directly with `hc$height`.


### Further reading

There is a lot more to TDA than what is discussed here --- we have covered only a small part of zero-dimensional topology. For a general introduction to TDA, see @carlsson2021topological.
...
There is a rich literature on multivariate kernel density estimation. Good starting points are @Scott2015 or @chacon2018multivariate. The idea of using a TDA-based bandwidth matrix is due to @lookout and @lookout2.
