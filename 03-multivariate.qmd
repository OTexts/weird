...
Topological data analysis (TDA) uses tools from topology to study data. Here, we introduce a few of the simplest ideas from TDA, which will prove useful in some anomaly detection methods.
...
The *Vietoris-Rips* complex is one type of $k$-simplicial complex, based on pairwise distances between points. Given a set of points and a proximity parameter $\delta > 0$, $k+1$ points within a distance of $\delta$ to each other form a $k$-simplex. For example, consider the eight points labelled $A$ to $H$ shown in @fig-tetrahedron, and suppose we choose $\delta=1.5$. The blue disks centred on each point have radius $\delta/2$; so overlapping disks indicate that their central points are within $\delta$ of each other.

Points $B$ and $H$ form an edge, but are disconnected from all other points because they are further than $\delta$ from the other points. The pair $A$ and $F$ also form an edge, and $D$ is connected to them via $F$. They don't form a triangle because $A$ and $D$ are too far apart. The points $CEG$ form a connected triangle.

```{r}
#| label: fig-tetrahedron
#| fig.cap: "An example of a Vietoris-Rips complex comprising eight points with $\\delta=1.4$. The blue disks have radius $\\delta/2$, so overlapping disks indicate that their central points are within $\\delta$ of each other. The resulting complex consists of three connected components."
#| message: false
#| warning: false
#| echo: false
X <- tribble(
...
Points that are connected in some way form a "connected component". There are three connected components in @fig-tetrahedron.
...
For example, as we increase $\delta$, topological features of the Vietoris-Rips complex change --- connected components merge, simplexes increase in dimension, holes are filled, and so on. Taking the small example of @fig-tetrahedron, we explore what happens as $\delta$ increases from 0.5 to 2.0. With a small value of $\delta$, all points are disconnected, and the Vietoris-Rips complex consists of 8 vertices. As $\delta$ increases, more points are connected to each other, and eventually, the complex consists of a single connected component.
...
The appearances and disappearances of these topological features are referred to as births and deaths. For example, a component is "born" at $\delta=0$, when it consists of a single point, and a component "dies" when existing components merge together. The death diameter of a connected component is known as the *persistence* of the feature. Births and deaths can be illustrated using a *barcode* diagram.
...
@fig-barcode contains the barcode of the point cloud shown in @fig-persistence, comprising a set of horizontal line segments, each denoting the birth and death of a connected component. For example, the shortest diameter corresponds to when $A$ and $F$ joined to form an edge --- when one of the separate vertex components died. The largest diameter corresponds to when all points became connected, when the $BH$ edge component died and joined the rest of the complex.

If there are $n$ observations, then there $n$ connected components, each born at diameter 0. The first death occurs when the closest two points merge, so the corresponding two features have the same death diameters. Rather than have a repeated bar, only $n-1$ bars are shown. As $\delta$ increases, the connected components disappear one by one, as they merge with neighbouring components, and eventually the number of connected components decreases to 1 when all observations are connected.

Components that continue for a large range of $\delta$ represent structural properties of the data (clusters) that are of interest to us. These are denoted by the longest lines in @fig-barcode.
...
An alternative approach to finding the bandwidth matrix uses ideas from topological data analysis (@sec-tda). First, we compute robust multivariate z-scores, $\bm{z}_i = \bm{U}(\bm{y}_i - \bm{m})$, $i=1,\dots,n$, where $\bm{m}$ is a vector of medians of $\{\bm{y}_1,\dots,\bm{y}_n\}$, and $\bm{U}$ is the Cholesky decomposition of $\bm{S}_{\text{OGK}}^{-1}$. Then we construct the barcode of the data cloud of z-scores using Vietoris-Rips complexes with increasing diameter $\delta$. From the barcode we obtain the sequence of death diameters $\{d_1,\dots,d_n\}$ for the connected components. Finally, we can set
$$
  \bm{H} = (d^*)^{2/m}\bm{S}_{\text{OGK}}
$$ {#eq-lookoutH}
where $d^*$ is the sample quantile $\hat{Q}(\gamma)$ computed from $\{d_1,\dots,d_n\}$ for a large value of $\gamma$. @lookout2 showed that this leads to a consistent density estimate, with good properties for anomaly detection, when $\gamma = 0.97$. We call this the "lookout bandwidth", because it is the basis of the lookout anomaly detection algorithm described in @sec-lookout. The `kde_bandwidth()` function will return this bandwidth when `method = "lookout"`. For univariate data, $\bm{S}_{\text{OGK}}$ is replaced by $s_{\text{Qn}}$ in @eq-lookoutH.
