...
As with univariate density estimation, when estimating the density in the tails of the distribution, it is usually better to have a larger bandwidth than those given by the rules above. When `multiplier = 2`, `kde_bandwidth()` will return the matrix $\bm{H}$ from @eq-gaussianH multiplied by 4 (because it is a variance, doubling the scale increases the matrix by a factor of four).
