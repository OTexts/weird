...
- A figure caption or prose sentence that names a specific colour-to-category mapping (e.g. "blue = Democrat, red = Republican") is a claim about what `scale_colour_*` actually produces, not just about the data. If the plotting code relies on ggplot's default discrete palette rather than an explicit `scale_colour_manual()`, the colour assigned to each category depends on the alphabetical order of the factor levels present in that particular plot, which can differ from external convention (e.g. red/blue for US political parties) and can even flip between panels if the majority/minority category changes. Actually check (`table()` the grouping variable, or read the rendered figure) which colour lands on which category rather than assuming the culturally-expected mapping; if the expected mapping matters to the reader, set it explicitly with `scale_colour_manual()` rather than trusting the default.
- An inline `` `r ...` `` expression that *assigns* a variable (e.g. `` `r n <- 100; n` ``) used so a later code chunk can reference that variable is a hidden, uncached dependency: knitr's chunk-level caching and `dependson` only track fenced code chunks, so a chunk that relies on a value set via inline assignment will not invalidate its cache if that inline value later changes. Prefer setting the value inside the code chunk itself (with the chunk showing the literal, matching the number already stated in the preceding prose, following the pattern used elsewhere in the book e.g. "$I=36$" in `10-timeseries.qmd`) rather than assigning it inline before the chunk.
...
- No visible code chunk should refer to objects or functions defined in invisible code chunks
